...
class luokehua(MyBaseTask, GameScript):
    """
    逆战未来 - 空间站塔防任务
    ...
    def run(self):
        ...
        while True:
            self.send_key('2',down_time=random.uniform(0.5, 1.5),after_sleep=random.uniform(12, 18))
            print("已按下键位")
            interval = random.uniform(1, 1000)
            if(interval>990):
                self.sleep(random.uniform(0,30))
                print("触发长休眠")
            print("休眠结束")
